...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	var calories, avarageSpeed float64
	avarageSpeed = meanSpeed(action, duration)
	calories = (runningCaloriesMeanSpeedMultiplier * avarageSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
	return calories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	height = height / cmInM
	calories = (walkingCaloriesWeightMultiplier*weight + (math.Pow(avarageSpeed, 2)/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return calories
}
...
func main() {
	fmt.Println(ShowTrainingInfo(4000, "Ходьба", 1, 85, 185, 50, 2))
